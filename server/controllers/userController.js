...
export const addUserRating = async (req, res) => {
  ...
  if (!courseId || !userId || !rating < 1 || rating > 5) {
    return res.status(204).json({ success: false, message: "Invalid Details" });
  }
  ...
  try {
    ...
    const exitingRatingIndex = course.courseRatings.findIndex(
      (r) => r.userId === userId
    );

    if (exitingRatingIndex > -1) {
      course.courseRating[exitingRatingIndex].rating = rating;
    } else {
